...
/**
 * The main GameLevel3 class for BIOSHOOK™, Level 3
 *
 * @author phil.nic
 */
public class GameLevel3 extends ApplicationAdapter implements Disposable {
    ...
    public void create () {
        ...
        assetManager.load ("maps/brain/brain.tmx", TiledMap.class);
        assetManager.finishLoading ();

        map = assetManager.get ("maps/brain/brain.tmx");

        new TMXToBox2d (world, map).populateBox2dWorld ();

        player = new Player (world, map, 1, 2, "neutrophil");
        rbcs = new RBCs (world, map, 50);
        glucoseTransporters = new GlucoseTransporters (world, map, 30);
        proteins = new Proteins (world, map, 20);
        antigens = new Antigens (world, map, 40, 0.85f, "purple");
